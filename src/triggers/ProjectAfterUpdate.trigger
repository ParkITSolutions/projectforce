trigger ProjectAfterUpdate on Project2__c (after update) {
            ...
            if( !ProjectUtil.flags.containsKey( 'deletingTaskOnly' ) || !ProjectUtil.flags.get( 'deletingTaskOnly' )){
	            List<Group> groupList = [ select id, name from Group where name in:groupsNames limit 1000 ];
	            List<GroupMember> groupMemberList = [ select UserOrGroupId, GroupId, id from GroupMember where GroupId in:groupList limit 1000 ];
	            
	            List<Group> go = [ Select g.Type, g.Name from Group g where Type = 'Organization' ];
	            
	            List<ProjectMember__c> projectMemberList = [ select project__c, id, User__c from ProjectMember__c where project__c in:idsProject limit 1000 ];
